...
public:
    ...
    кстати, можно переложить эту задачу на фронтендера
    сейчас бэк принимает сообщение и пересылает его как получателям, так и отправителю
    но отправитель может сразу записывать сообщение себе, а бэк будет только получателям перенаправлять...
    но пока так, копия пусть будет  
    */
